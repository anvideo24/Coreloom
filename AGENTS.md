...
1. **일하는 방식** — `C:\dev\working-method` (GitHub `anvideo24/working-method`, 공개).
   그 `AGENTS.md`가 정한 순서(`HOW` → `LESSONS` → `RULES`)로 읽는다.
2. **개인 맥락·공용 규칙** — `C:\dev\base`의 `AGENTS.md`.
3. 그다음 **이 파일**과 이 저장소의 `RULES.md`.

겹치면 **이 저장소가 이 제품의 정본**이다. 공용을 여기에 복사해 두지 않는다 — 복사하는 순간
원본이 둘이 되고, 둘은 반드시 그리고 조용히 갈라진다. 새 작업을 잡거나 작업 종류가 바뀌면 다시 읽는다.
