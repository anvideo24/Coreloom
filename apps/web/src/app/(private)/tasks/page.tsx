...
export default async function TasksPage() {
  ...
  const { projects, tasks, schedule } = await listFounderTasks(session.founder.id);
    ...
    <main className="operations-shell">
      <header className="operations-header">
        <div>
          ...
          <h1>업무·일정</h1>
          <p>프로젝트에 업무를 연결하고 기한과 완료 조건을 남깁니다. 완료는 대표가 조건을 확인한 뒤에만 처리하며, Recho 캘린더 연동은 포함하지 않습니다.</p>
        </div>
      </header>
      {projects.length === 0 ? (
      ...
      <section className="quote-list" aria-label="업무 목록">
        ...
        {tasks.length === 0 ? <p className="empty-state">아직 등록된 업무가 없습니다.</p> : tasks.map((task) => (
          <a className="quote-row" href={`/tasks/${task.id}`} key={task.id}>
            <div>
              <p>{task.clientName} · {task.projectName} · 기한 {task.dueDate}</p>
              <h3>{task.title}</h3>
            </div>
            <strong>{taskStatusLabels[task.status]}</strong>
          </a>
