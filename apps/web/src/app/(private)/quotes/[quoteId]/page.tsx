...
export default async function QuoteDetailPage({ params }: { params: Promise<{ quoteId: string }> }) {
  ...
  return <main className="operations-shell"><header className="operations-header"><div><p className="auth-eyebrow">CORELOOM / QUOTE HISTORY</p><h1>{latest.title}</h1><p>{detail.quote.clientName} · 현재 v{latest.versionNumber}</p></div><div className="quote-header-links">{contract ? <Link className="text-link" href={`/contracts/${contract.id}`}>연결된 계약</Link> : null}<Link className="text-link" href="/quotes">견적서 목록</Link></div></header>
    {contract ? null : <section className="quote-editor-card"><p className="setup-code">계약 초안</p><p className="form-help">현재 견적 버전을 계약 초안으로 옮깁니다. 날인 원본 위치와 체결 확정은 계약 화면에서 이어갑니다.</p><form action={createContractFromQuoteAction}><input name="quoteVersionId" type="hidden" value={latest.id} /><button className="auth-submit" type="submit">이 버전으로 계약 초안 만들기</button></form></section>}
