...
export function AgentAccessSettings({ agentId, onDirty }: { agentId: string; onDirty?: (dirty: boolean) => void }) {
  ...
  return <section className="agent-access-settings" aria-label="에이전트 조회 권한">
    ...
    {data ? <form className="agent-chat-settings" onSubmit={async (event) => {
      ...
      if (expansion.length && !window.confirm(`${expansion.join("\n")}\n이 범위만 다음 요청부터 조회할 수 있습니다. 저장할까요?`)) return;
      setBusy(true); setNotice("");
      try { await saveAgentAccessAction(agentId, { permissions: data.permissions, roots: nextRoots }); setSavedPermissions(data.permissions); setSavedRoots(nextRoots); onDirty?.(false); setNotice("조회 권한을 저장했습니다. 다음 요청부터 적용됩니다."); }
