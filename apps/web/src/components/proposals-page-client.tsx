...
export function ProposalsPageClient({
  ...
  const canCreate = evidence.length > 0;
    ...
    <>
      <header className="operations-header">
        <div>
          ...
          <h1>AI 제안</h1>
          <p>
            근거 기록에 연결된 현재 합의·다음 할 일·위험 초안을 등록하고, 대표가 확정하거나 반려합니다. 확정 전에는 공식
            결정이 아닙니다. 자동 생성, 계약 체결, 외부 발송은 이 기능에 포함되지 않습니다.
          </p>
        </div>
        <CreateIconButton disabled={!canCreate} label="새 제안" onClick={openCreate} />
      </header>
      ...
      {!canCreate ? (
        <section className="empty-state quote-empty">
          ...
          <p>
            근거가 없는 AI 요약은 확정 후보로 올리지 않습니다. Recho 메일·통화·회의를 프로젝트에 연결한 뒤에 제안을
            등록하세요.
          </p>
      ...
      <section aria-label="확인할 제안" className="quote-list">
        ...
        {pending.length === 0 ? (
          ...
          pending.map((proposal) => (
            <a className="quote-row" href={`/proposals/${proposal.id}`} key={proposal.id}>
              <div>
                <p>
                  {proposal.clientName} · {proposal.projectName} · {aiProposalKindLabels[proposal.kind]} ·{" "}
                  {proposal.occurredOn}
                </p>
                <h3>{proposal.body}</h3>
              </div>
      ...
      <section aria-label="결정 이력" className="quote-list">
        <div className="list-heading">
          <div>
        ...
        {decided.length === 0 ? (
          <p className="empty-state">확정하거나 반려한 제안이 없습니다.</p>
        ) : (
          decided.map((proposal) => (
            <a className="quote-row" href={`/proposals/${proposal.id}`} key={proposal.id}>
              <div>
                <p>
                  {proposal.clientName} · {proposal.projectName} · {aiProposalKindLabels[proposal.kind]} ·{" "}
                  {isOfficialDecision(proposal.status) ? "공식 결정" : "공식 결정 아님"}
                </p>
                <h3>{proposal.body}</h3>
              </div>
              <strong>{aiProposalStatusLabels[proposal.status]}</strong>
      ...
      {canCreate ? (
        <CreatePanel onClose={close} open={open} size="wide" title="새 제안">
          <form action={createAiProposalAction} className="quote-form">
            <p className="form-help quote-form-full">
              제안 본문은 AI가 만든 초안을 대표가 옮겨 적는 것이며, Recho 원문이 아닙니다.
            </p>
            <p className="setup-code quote-form-full">연결</p>
            <label className="quote-form-full">
              근거 기록
              <select defaultValue={evidence[0]?.id} name="evidenceId" required>
                {evidence.map((item) => (
                  <option key={item.id} value={item.id}>
                    {item.clientName} · {item.projectName} · {item.occurredOn} · {item.title}
                  </option>
                ))}
              </select>
            </label>

            <p className="setup-code quote-form-full">제안</p>
            <label>
              종류
              <select defaultValue="agreement" name="kind">
                {aiProposalKinds.map((kind) => (
                  <option key={kind} value={kind}>
                    {aiProposalKindLabels[kind]}
                  </option>
                ))}
              </select>
            </label>
            <label className="quote-form-full">
              ...
              <textarea
                name="body"
                placeholder="근거 원문을 확인한 뒤, 현재 합의·다음 할 일 또는 위험을 적습니다."
                required
              />
            </label>
